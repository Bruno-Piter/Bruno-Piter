...
def card(p, x, y, idx):
    ...
    label = repo if len(repo) <= 52 else repo[:51] + "…"
    a(f'<text x="16" y="19" font-size="10" fill="{MUTED}"><tspan fill="{CYAN}">&#8226;</tspan> {esc(label)}</text>')
    ...
    stars = p.get("stars", 0)
    when = rel_time(p.get("pushed_at")) if p.get("pushed_at") else "soon"
    a(f'<text x="68" y="155" font-size="11" fill="{MUTED}">'
      f'<tspan fill="{CYAN}">&#9733;</tspan> {stars}'
      f'<tspan fill="{DIM}" dx="14">updated {esc(when)}</tspan></text>')
